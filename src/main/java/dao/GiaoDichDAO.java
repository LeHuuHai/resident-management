...
public class GiaoDichDAO implements DAOInterface<GiaoDich>{
    ...
    public boolean insert(GiaoDich t){
       try {
            ...
            String sql = "INSERT INTO GIAODICH (GiaoDichID, KhoanPhiID, SoTien, HoKhauID, ThoiGianGiaoDich)" +
                    " VALUES (" + t.getMaGiaoDich()+ ", " + t.getMaKhoanPhi()+ 
                    ", " + t.getSoTien()+ ", " + t.getMaHoKhau() + ", '" + t.getThoiGian() + "');";
//            ans: so dong bi thay doi trong sql
    ...
    @Override
    public boolean delete(GiaoDich t){
        try {
            Connection connection = JDBCUtil.getConnection();
            Statement st = connection.createStatement();
